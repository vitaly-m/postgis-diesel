...
macro_rules! impl_point_to_sql_geography {
	($($type:ty),+) => {
		$(
			#[cfg(feature = "diesel")]
			impl<B> diesel::serialize::ToSql<Geography, B> for $type
			where
				B: diesel::backend::Backend,
				Self: diesel::serialize::ToSql<Geometry, B>,
			{
			...
			#[cfg(feature = "diesel")]
			impl<B> diesel::deserialize::FromSql<Geography, B> for $type
			where
				B: diesel::backend::Backend,
				Self: diesel::deserialize::FromSql<Geometry, B>,
			{
...
impl_point_to_sql_geography!(Point, PointZ, PointM, PointZM);
