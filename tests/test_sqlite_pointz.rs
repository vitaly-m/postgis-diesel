//! Submodule to test the use the `PointZ` type with SQLite backend.
...
fn new_point(x: f64, y: f64, z: f64) -> PointZ {
    PointZ::new(x, y, z, Some(4326))
}

fn establish_sqlite_connection() -> SqliteConnection {
    // We delete the database file if it exists
    let _ = std::fs::remove_file("test_pointz.sqlite");

    let mut conn = SqliteConnection::establish("test_pointz.sqlite").expect("Error connecting to sqlite");
    INIT.call_once(|| {
