//! Submodule to test the use the `PointM` type with SQLite backend.
...
fn new_point(x: f64, y: f64, z: f64) -> PointM {
    PointM::new(x, y, z, Some(4326))
}

fn establish_sqlite_connection() -> SqliteConnection {
    // We delete the database file if it exists
    let _ = std::fs::remove_file("test_pointm.sqlite");

    let mut conn = SqliteConnection::establish("test_pointm.sqlite").expect("Error connecting to sqlite");
    INIT.call_once(|| {
...
(
    ...
    point             BLOB NOT NULL
        ...
        let north_sample = NewGeometrySample {
            ...
            point: new_point(72.0, 64.0, 30.0),
        };
        let east_sample = NewGeometrySample {
            name: "eastern".to_string(),
            point: new_point(64.0, 64.0, 30.0),
        };
